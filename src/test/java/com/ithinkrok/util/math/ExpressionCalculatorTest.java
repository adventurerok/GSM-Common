...
public class ExpressionCalculatorTest {
    ...
    public void shouldHandleVariablesCorrectly(String sum, double x, double y, double result) {
        ...
        Mockito.doThrow(new RuntimeException("Should only be asking for x and y")).when(variables).getVariable(any());
    ...
    @Test
    @DataProvider(value = {"32*4-7", "x+y", "0", "val"}, splitBy = ";")
    public void toStringShouldNotBeNull(String sum) {
        ExpressionCalculator calc = new ExpressionCalculator(sum);

        assertThat(calc.toString()).isNotNull();
    }
